        ...
        <div id='historyGridContainer' className="md:grid-cols-history-large grid-cols-history-small grid relative my-12 ">
            ...
            <div className=" flex justify-around flex-col space-y-24 2xl:space-y-24 px-4 text-left">
                ...
                <div id='2022' className="flex flex-col space-y-4 2xl:space-y-8 md:w-2/3">
                    ...
                    <p className="text-left 2xl:text-2xl">ISSIMI expands to Italy with the launch of ISSIMI Italia and the opening of its new showroom in Verona. The company becomes fully operational in Italy, offering its services to local collectors and enthusiasts.</p>
                    <p className="text-left 2xl:text-2xl">Two Pagani Huayra BCs transacts on our marketplace in Italy.</p>
                </div>
            </div>
        </div>
    )
...
function History2019({ mobile }: { mobile: boolean }) {
    return (
        <div id={mobile ? "mobile2019" : "2019"} className={"space-y-4 2xl:space-y-8  md:float-right md:ml-auto md:w-2/3 text-left" + (mobile ? " flex flex-col md:hidden text-left" : 'hidden md:flex md:flex-col text-right')}>
            ...
            <p className="2xl:text-2xl">ISSIMI, Inc continues to grow as a trusted platform for buying and selling collector cars. As part of our commitment to providing valuable content and insights to our community, we launched our own YouTube channel.</p>
            <p className="2xl:text-2xl">Revenue doubles from the first year, reflecting the trust and confidence that our customers have in us as their preferred partner. </p>
            <p className="2xl:text-2xl">Ferrari 333 SP transacts on our marketplace, from Netherlands to Denmark.</p>
        </div>
    )
}
...
function History2021({ mobile }: { mobile: boolean }) {
    return (
        <div id={mobile ? "mobile2021" : "2021"} className={" space-y-4 2xl:space-y-8  md:float-right md:ml-auto md:w-2/3 " + (mobile ? " flex flex-col md:hidden text-left" : 'hidden md:flex md:flex-col text-right')} >
            ...
            <p className="2xl:text-2xl">ISSIMI Factory begins collaborations with prestigious car manufacturers Koenigsegg and Rimac, reflecting our commitment to full car collection services. As our reputation for quality and transparency continues to spread, revenue grows 63% from the previous year.</p>
            <p className="2xl:text-2xl">McLaren F1 transacts on our marketplace, from UK to California.</p>
        </div>
    )
}